...
export default function AdministrasiPersuratanPage(){
  ...
  return (
    <PageShell title="Administrasi Persuratan" description="Kelola data administrasi — buat, lihat, dan hapus. Form dapat ditambah terus sesuai kebutuhan." breadcrumbs={[{label:"Persuratan"},{label:"Administrasi"}]} actions={<Link href="/administrasi-persuratan/new"><Button><Plus size={14}/> Buat Administrasi</Button></Link>}>
      <DataTable data={data} total={total} page={page} limit={10} totalPages={Math.ceil(total/10)} onPageChange={(p)=>{setPage(p); load(p,search)}} onSearch={(s)=>{setSearch(s); setPage(1); load(1,s)}} searchPlaceholder="Cari administrasi..." loading={loading}
        columns={[
          {key:"name", header:"Nama", render:(r)=><div className="flex items-center gap-2"><div className="w-7 h-7 rounded bg-emerald-100 flex items-center justify-center text-emerald-700"><ClipboardList size={12}/></div><div><div className="font-bold text-xs">{r.name}</div><div className="text-[11px] text-[#6b7280]">{r.description||"-"}</div></div></div>},
