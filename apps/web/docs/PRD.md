...
- **DB:** `prisma/schema.prisma` **25 models** (RBAC 18 + Global Tables 2 + Persuratan 5) sesuai spec `/knowledge:backup`. Hitung `grep -c "^model "` saat backup menampilkan 19 models terhitung real di file (RBAC 10 + junctions termasuk + Global 2 + Persuratan 5 + ActivityLog/Setting) — selisih karena `ai_*` legacy sudah diganti file store. Dokumen ini memakai **25 models** untuk kompatibilitas + catatan real 19 di `database.md`.
...
### 2026-09-20 — Knowledge Backup apps/web (Global Tables 13 tipe + persuratan stabil)
- **Discovery:** `prisma/schema.prisma` 25 models (spec — real 19 vs spec 25 dicatat di database.md), `lib/prisma.ts` singleton, `DATABASE_URL="file:./dev.db"` (`prisma7.config.ts`), `lib/services/global-tables.service.ts` 451 (13 tipe `dyn_*`), `lib/renderer/operationEngine.ts` 161 (`++` `""` `* / + -`), `app/global-tables/page.tsx` GUI 13 tipe, `app/dyn/[table]/page.tsx` search/options/order, persuratan 4 tahap (`app/components-persuratan`, `app/templates-persuratan`, `app/administrasi-persuratan`, `app/hasil-persuratan`), Office Doc paper `#e8ecef` + `shadow-[0_2px_16px_rgba(0,0,0,0.12)]` di `components/forms/PersuratanTemplateForm.tsx`, `components/layout/AppLayout.tsx` 4 grup, `app/globals.css` tokens `#0075de` HSL 210 100% 44% + `#f6f5f4` + `#e6e6e6`, `next.config.ts` rewrites `/builder`→`/generated/surat-platform/builder`, `app/api/global-tables`, `app/api/dyn`, `app/api/persuratan`, `middleware.ts` missing (konflik dicatat). Update header `Last Backup: 2026-09-20`.
