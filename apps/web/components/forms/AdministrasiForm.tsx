...
export default function AdministrasiForm({ mode, id }: { mode:"create"|"edit"; id?: string }){
  ...
  return (
    <div className="space-y-3 bg-white rounded-[8px] border border-[#e6e6e6] p-3">
      ...
      <Card>
        <CardHeader className="pb-2"><CardTitle className="text-[13px] flex items-center justify-between"><span className="flex items-center gap-2"><Type size={14}/> Data Surat</span><Button size="sm" variant="outline" onClick={addField}><Plus size={12}/> Tambah Field</Button></CardTitle><div className="text-xs text-[#6b7280]">Tambahkan field sesuai kebutuhan — nama akan otomatis diberi awalan langkah</div></CardHeader>
        <CardContent className="space-y-3">
          {form.fields.map((f,idx)=>(
            <div key={idx} className="flex gap-1.5 items-end border border-[#e6e6e6] rounded-[8px] p-2.5 bg-[#fafafa]">
              <div className="flex-1"><Label className="text-[11px]">Nama Data</Label><Input value={f.name} onChange={e=>updateField(idx,{name:e.target.value})} placeholder="judul" className="h-7 text-[13px] font-mono" /><div className="text-[11px] text-[#6b7280] mt-1">Akan jadi <code className="bg-white px-1 rounded border">step1_{f.name||"nama"}</code></div></div>
          ...
          {form.fields.length===0 && <div className="text-xs text-[#6b7280] p-2.5 text-center border-2 border-dashed rounded-[8px]">Belum ada field — klik Tambah Field</div>}
        </CardContent>
      </Card>

      <Card>
        <CardHeader className="pb-2"><CardTitle className="text-[13px] flex items-center gap-1.5"><FileStack size={14}/> Steps — Template Administrasi</CardTitle><div className="text-xs text-[#6b7280]">Pilih template untuk setiap step, dapat tambah step baru terus</div></CardHeader>
        <CardContent className="space-y-3">
          {steps.map((s,idx)=>(
            <div key={idx} className="flex gap-1.5 items-center border rounded-[8px] p-2.5 bg-white">
