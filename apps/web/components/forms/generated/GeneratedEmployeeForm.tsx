...
export default function GeneratedEmployeeForm({ mode, id }: { mode:"create"|"edit"; id?: string }){
  ...
  return (
    <div className="space-y-2.5 bg-white rounded-[8px] border border-[#e6e6e6] p-3">
      ...
      <div className="flex justify-end gap-1.5 border-t pt-2.5"><Button variant="outline" onClick={()=>router.push("/generated/surat-platform/employees")}>Batal</Button><Button onClick={handleSubmit}>Simpan</Button></div>
    </div>
  )
}
